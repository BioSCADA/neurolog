...
namespace ThinkGearNET
{
	public class ThinkGearWrapper
	{
		...
		public bool Connect(string comPort, int baud, bool async, bool trace)
		{
			...
			// connect to the device
                ...
                connect = ThinkGear.TG_Connect(_connectionId, @"\\.\" + comPort, baud, ThinkGear.STREAM_PACKETS);
	...
	public class ThinkGearState
	{
		...
		public override string ToString()
		{
            ...
            sb.AppendFormat("{0};", PacketsRead);
            sb.AppendFormat("{0};", Battery);
            sb.AppendFormat("{0};", PoorSignal);
            sb.AppendFormat("{0};", Attention);
            sb.AppendFormat("{0};", Meditation);
            sb.AppendFormat("{0};", Raw);
            sb.AppendFormat("{0};", Delta);
            sb.AppendFormat("{0};", Theta);
            sb.AppendFormat("{0};", Alpha1);
            sb.AppendFormat("{0};", Alpha2);
            sb.AppendFormat("{0};", Beta1);
            sb.AppendFormat("{0};", Beta2);
            sb.AppendFormat("{0};", Gamma1);
            sb.AppendFormat("{0};", Gamma2);
            sb.AppendFormat("{0};", BlinkStrength);
            return sb.ToString();
		}
	}
